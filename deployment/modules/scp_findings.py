...
def get_scp_findings() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for scp in all_scps:
            ...
            wildcard_findings = _check_wildcard_actions(scp_id, scp_name, policy_document)
            findings.extend(wildcard_findings)

            # Check for overly permissive NotAction
            notaction_findings = _check_wildcard_notactions(scp_id, scp_name, policy_document)
        ...
        consistency_findings = _check_policy_attachment_consistency(org, root_id, all_scps)
