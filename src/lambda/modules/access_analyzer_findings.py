    ...
    try:
        ...
        for page in paginator.paginate(analyzerArn=analyzer_arn, filter={'status': {'eq': ['ACTIVE']}}):
            for finding in page['findings']:
                ...
                target_types = ['AWS::S3::Bucket', 'AWS::IAM::Role', 'AWS::KMS::Key', 'AWS::Lambda::Function', 'AWS::SQS::Queue']
                if resource_type in target_types:
                    findings.append({
                        'resource_id': finding.get('resource', 'Unknown'),
                        'resource_type': resource_type,
                        'service': resource_type.split('::')[1] if '::' in resource_type else 'AccessAnalyzer',
